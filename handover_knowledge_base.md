...
> 在 4K 屏等高 DPI 设备上，或者在用户缩放终端界面（Zoom In/Out）时，Monospace 字体的单字宽度可能计算出带小数点的像素值（非整数）。
> * **避免行为**：**绝对不要**采用 `字符个数 * 字符像素宽度` 这样的粗暴公式直接累加计算光标的物理 `x` 坐标，这会导致在高分屏上行尾的字符和光标出现多个像素的肉眼可见抖动或间距偏离。
> * **正确做法**：始终依赖 Qt `QFontMetricsF` 或者通过 Fragment 的实际占位宽度（Layout Row 轮廓）反向查询绝对物理位置。

> [!CAUTION]
> ### 6. Windows ReadFile 阻塞引起的程序退出死锁（生命周期同步）
> 在 [conptyprocess.cpp](file:///F:/B_My_Document/GitHub/qt-terminal-widget/src/pty/conptyprocess.cpp) 的析构和 `kill()` 逻辑中，我们设计了先关闭管道句柄、再等待读线程退出的时序：
