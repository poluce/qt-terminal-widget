...
bool UnixPtyProcess::startProcess(const QString &shellPath, QStringList environment, qint16 cols, qint16 rows)
{
    ...
    char *ptsName = ptsname(m_shellProcess.m_handleMaster);
    if (!ptsName)
    {
    ...
    m_shellProcess.m_handleSlaveName = QString(ptsName);
    ...
    if (rc != 0)
    {
        m_lastError = QString("UnixPty Error: unable to set associated params -> %1").arg(strerror(errno));
        kill();
        return false;
    }
...
void ShellProcess::setupChildProcess()
{
    ...
    strncpy(utmpxInfo.ut_user, qgetenv("USER").constData(), sizeof(utmpxInfo.ut_user) - 1);
    ...
    strncpy(utmpxInfo.ut_id, d + strlen(d) - qMin((size_t)strlen(d), sizeof(utmpxInfo.ut_id) - 1), sizeof(utmpxInfo.ut_id) - 1);
